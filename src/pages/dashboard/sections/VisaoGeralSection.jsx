  ...
  return (
    <div className="space-y-6">
      <div className="grid md:grid-cols-3 gap-4 items-start">
        <div className="relative bg-dark-200 border border-gray-800 rounded-custom p-5"><div className="text-xs text-gray-500 mb-2">CANCELAMENTOS HOJE</div><div className="absolute right-4 top-4 inline-flex items-center rounded-full border border-gray-700 bg-transparent px-3 py-1 text-xs text-primary">{Number(metricsHoje?.today?.taxa_cancelamento || 0).toFixed(1)}%</div><div className="text-3xl font-normal text-white">{Number(metricsHoje?.today?.cancelados || 0)}</div></div>
        <div className="bg-dark-200 border border-gray-800 rounded-custom p-5"><div className="text-xs text-gray-500 mb-2">CONCLUÍDOS HOJE</div><div className="text-3xl font-normal text-white">{Number(metricsHoje?.today?.concluidos || 0)}</div><div className="mt-2 inline-flex items-center rounded-full border border-gray-700 bg-transparent px-3 py-1 text-xs text-primary">R$ {Number(metricsHoje?.today?.ticket_medio || 0).toFixed(2)}</div></div>
      ...
      <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4 items-start">
          ...
          <div className="relative bg-dark-100 border border-gray-800 rounded-custom p-4"><div className="text-xs text-gray-500 mb-1">CANCELADOS</div><div className="absolute right-3 top-3 inline-flex items-center rounded-full border border-yellow-400/30 bg-yellow-400/10 px-2.5 py-1 text-xs text-yellow-400">{Number(metricsDia?.selected_day?.taxa_cancelamento || 0).toFixed(1)}%</div><div className="text-xl font-normal text-red-400">{Number(metricsDia?.selected_day?.cancelados || 0)}</div></div>
