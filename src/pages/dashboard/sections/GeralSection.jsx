    ...
    <div className="space-y-6">
      ...
      <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
        ...
        <div className="mt-2 bg-dark-100 border border-gray-800 rounded-custom p-4">
          ...
          {souDono && faturamentoPorProfissionalPeriodo.length > 0 && (<div className="mt-3 flex snap-x snap-mandatory gap-3 overflow-x-auto scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{faturamentoPorProfissionalPeriodo.map(([nome, valor, concluidos, variacao]) => (<div key={String(nome)} className="relative shrink-0 basis-full snap-start bg-dark-200 border border-gray-800 rounded-custom p-4 pr-24 lg:basis-[calc((100%-1.5rem)/3)]"><PeriodRevenueBadge value={variacao} /><div className="text-xs text-gray-500 mb-1">PROFISSIONAL</div><div className="font-normal text-white uppercase">{String(nome || '—')}</div><div className="text-primary font-normal mt-1">R$ {Number(valor || 0).toFixed(2)}</div><div className="mt-2 inline-flex items-center rounded-full border border-gray-700 bg-transparent px-3 py-1 text-xs text-gray-500">{Number(concluidos || 0)} CONCLUÍDOS</div></div>))}</div>)}
        </div>
      </div>
    </div>
  );
