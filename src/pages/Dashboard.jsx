...
function TemaToggle({ value, onChange, loading }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className={`text-xs font-normal uppercase transition-colors ${!isLight ? 'text-primary' : 'text-gray-600'}`}>
        DARK
      </span>
      <button
        type="button"
        ...
        className={`relative inline-flex h-7 w-14 shrink-0 items-center rounded-full border transition-all duration-300 focus:outline-none ${
          loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
        } ${isLight ? 'bg-white border-gray-300' : 'bg-dark-200 border-gray-700'}`}
      >
        <span
          className={`inline-block h-5 w-5 rounded-full shadow-md transition-all duration-300 ${
            isLight ? 'translate-x-7 bg-gray-900' : 'translate-x-1 bg-primary'
          }`}
        />
      </button>
      <span className={`text-xs font-normal uppercase transition-colors ${isLight ? 'text-primary' : 'text-gray-600'}`}>
...
function getValorServico(servico) {
  const preco = Number(servico?.preco ?? 0);
  const promoRaw = servico?.preco_promocional;
...
function getValorAgendamento(a) {
  ...
  return getValorServico(a?.servicos);
}
...
export default function Dashboard({ user, onLogout }) {
  ...
  const loadHoje = async (negocioId) => {
    ...
    try {
      ...
      const payload = extractRpcPayload(data, 'get_dashboard_today');
      setMetricsHoje(payload);
    } catch (e) {
      console.error('loadHoje error:', e);
      setMetricsHoje(null);
    } finally {
  ...
  const loadDia = async (negocioId, dateISO) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('get_dashboard_day', {
        p_negocio_id: id,
        p_date: date,
      });
      if (error) throw error;

      const payload = extractRpcPayload(data, 'get_dashboard_day');
      setMetricsDia(payload);
    } catch (e) {
      console.error('loadDia error:', e);
      setMetricsDia(null);
    } finally {
  ...
  const loadPeriodo = async (negocioId, refDateISO, periodo) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('get_dashboard_period', {
        p_negocio_id: id,
        p_ref_date: refDate,
        p_periodo: per,
      });
      ...
      const payload = extractRpcPayload(data, 'get_dashboard_period');
      setMetricsPeriodoData(payload);
    } catch (e) {
      console.error('loadPeriodo error:', e);
  ...
  useEffect(() => {
    ...
    const day = faturamentoData || hoje;
    loadDia(negocio.id, day);
  }, [negocio?.id, hoje]);
    ...
    const { data, error: qErr } = await supabase
      ...
      .select(`
        *,
        ...
        horario_inicio,
        horario_fim,
        servicos (nome, preco, preco_promocional),
  ...
  const loadMoreHistorico = async () => {
    if (historicoLoadingMore) return;
    ...
    try {
      ...
      console.error('loadMoreHistorico error:', e);
    } finally {
  ...
  const loadData = async (dataRef) => {
    ...
    if (dataRef === true) {
      try {
        dataRef = await fetchNowFromDb();
      } catch (e) {
        dataRef = String(serverNow?.date || hoje || '');
      }
    }
    ...
    try {
      ...
      const [galeriaResult, profissionaisResult] = await Promise.all([
        ...
        supabase
          .rpc('get_profissionais_com_status', {
      ...
      const dataHoje = (typeof dataRef === 'string' && dataRef)
        ? dataRef
        : String(serverNow?.date || hoje || '');

      const [servicosResult, agendamentosResult] = await Promise.all([
        supabase
          .from(VIEWS.servicos)
  ...
  const uploadLogoNegocio = async (file) => {
    ...
    try {
      ...
      const { error: dbErr } = await supabase
        .from('negocios')
        .update({ logo_path: `logos/${filePath}` })
        .eq('id', negocio.id)
        .eq('owner_id', user.id);
      ...
      for (const file of Array.from(files)) {
        ...
        const { error: dbErr } = await supabase
          ...
          .insert({
            ...
            path: `galerias/${filePath}`
          });

        if (dbErr) {
          console.error('insert galeria error:', dbErr);
          await uiAlert('dashboard.gallery_upload_error', 'error');
        }
      }
  ...
  const createServico = async (e) => {
    ...
    try {
      ...
      if (!payload.nome) throw new Error('Nome do servico e obrigatorio.');
  ...
  const updateServico = async (e) => {
    ...
    try {
      ...
      if (!toUpperClean(formServico.nome)) throw new Error('Nome do servico e obrigatorio.');
      if (!formServico.profissional_id) throw new Error('Selecione um profissional.');
      if (!toNumberOrNull(formServico.duracao_minutos)) throw new Error('Duracao invalida.');
      if (preco == null) throw new Error('Preco invalido.');
      if (promo != null && promo >= preco) throw new Error('Preco de oferta deve ser menor.');
  ...
  const deleteServico = async (id) => {
    ...
    try {
      ...
      await uiAlert('dashboard.service_delete_error', 'error');
    }
  };
  ...
  const createProfissional = async (e) => {
    ...
    try {
      ...
      await uiAlert('dashboard.professional_create_error', 'error');
    }
  };
  ...
  const updateProfissional = async (e) => {
    ...
    try {
      ...
      const msg = String(e2?.message || '');
      if (msg.includes('profissional_almoco_bloqueado')) {
        await uiAlert('dashboard.professional_lunch_blocked', 'warning');
      } else if (msg.includes('profissional_dia_bloqueado')) {
        await uiAlert('dashboard.professional_workday_blocked', 'warning');
      } else {
  ...
  const toggleAtivoProfissional = async (p) => {
    try {
      if (p.ativo === undefined) {
        await uiAlert('dashboard.professional_missing_active_column', 'error');
        return;
      }
      ...
      if (!novoAtivo) {
        const r = await uiPrompt('dashboard.professional_inactivate_reason', {
          variant: 'warning',
          placeholder: '',
          initialValue: ''
        });
  ...
  const cancelarAgendamentoProfissional = async (id) => {
    ...
    try {
      const { error: updErr } = await supabase
        .from('agendamentos')
        .update({ status: 'cancelado_profissional' })
        .eq('id', id)
        .eq('negocio_id', negocio.id);
      ...
      console.error('cancelarAgendamentoProfissional:', e);
  ...
  const agendamentosHoje = useMemo(
    () => agendamentos.filter(a => sameDay(getAgDate(a), hoje)),
    [agendamentos, hoje]
  );

  const hojeValidos = useMemo(
    () => agendamentosHoje.filter(a => !isCancelStatus(a.status)),
    [agendamentosHoje]
  );
  ...
  const faturamentoPorProfissionalHoje = useMemo(() => {
    ...
    const pairs = arr
      .map((x) => {
        ...
        if (!nome) return null;
        if (!Number.isFinite(valor)) return [nome, 0];
        return [nome, valor];
      })
      .filter(Boolean);
  ...
  if (error || !negocio) return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-dark-100 border border-red-500/50 rounded-custom p-8 text-center">
        ...
        <button onClick={() => loadData(true)} className="w-full px-6 py-3 bg-primary/20 border border-primary/50 text-primary rounded-button mb-3 font-normal uppercase">
          TENTAR NOVAMENTE
        </button>
          ...
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <label className="inline-block">
                ...
                />
                <span className={`inline-flex items-center justify-center text-center rounded-button font-normal border transition-all uppercase focus:outline-none focus:ring-0 focus:ring-offset-0 ${
                  logoUploading
          ...
          <div className="bg-gradient-to-br from-green-500/20 to-emerald-600/20 border border-green-500/30 rounded-custom p-6">
            ...
            <div className="text-3xl font-normal text-white mb-1">
              {metricsLoadingHoje ? (
                ...
                <>R$ {Number(metricsHoje?.today?.faturamento || 0).toFixed(2)}</>
              )}
            </div>
          </div>
              ...
              <div key={i} className="announcement-bar-track flex items-center shrink-0 whitespace-nowrap" aria-hidden={i === 2}>
                {[...Array(12)].map((_, index) => (
                  <div key={index} className="flex items-center">
                    ...
                    <Link to={`/v/${negocio.slug}`} target="_blank" className="text-black font-normal text-sm uppercase hover:underline underline-offset-4 transition-all mx-4">
                      VER VITRINE
                    </Link>
                    <span className="text-black mx-4">●</span>
                    <a href={SUPORTE_HREF} target="_blank" rel="noreferrer" className="text-black font-normal text-sm uppercase hover:underline underline-offset-4 transition-all mx-4">
                      SUPORTE
                    </a>
                    <span className="text-black mx-4">●</span>
                  </div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                    ...
                    <div className="text-xs text-gray-300 mt-1">
                      TAXA: <span className="text-primary">{Number(metricsHoje?.today?.taxa_cancelamento || 0).toFixed(1)}%</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                    ...
                    <div className="text-xs text-gray-300 mt-1">
                      TICKET MÉDIO: <span className="text-primary">R$ {Number(metricsHoje?.today?.ticket_medio || 0).toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                    ...
                    {proximoAgendamento ? (
                      <>
                        ...
                        <div className="text-xs text-gray-500 mt-1">{proximoAgendamento.servicos?.nome}</div>
                      </>
                    ) : (
                ...
                <div className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                  ...
                  <div className="text-3xl font-normal text-white mb-2">
                    {metricsLoadingDia ? (
                      ...
                      <>R$ {Number(metricsDia?.selected_day?.faturamento || 0).toFixed(2)}</>
                    )}
                  </div>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      ...
                      <div className="bg-dark-200 border border-gray-800 rounded-custom p-4">
                        ...
                        <div className="text-xl font-normal text-primary">
                          {metricsLoadingPeriodo ? '...' : `R$ ${Number(metricsPeriodoData?.period?.faturamento || 0).toFixed(2)}`}
                        </div>
                      </div>
                      <div className="bg-dark-200 border border-gray-800 rounded-custom p-4">
                        <div className="text-xs text-gray-500 mb-1">MÉDIA POR ATENDIMENTO</div>
                        <div className="text-xl font-normal text-white">
                          {metricsLoadingPeriodo ? '...' : `R$ ${Number(metricsPeriodoData?.period?.media_por_atendimento || 0).toFixed(2)}`}
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-primary/20 hover:bg-primary/30 border border-primary/50 text-primary rounded-button text-sm font-normal uppercase"
                  >
                    ATUALIZAR
                  </button>
                </div>
                              ...
                              <div key={a.id} className="bg-dark-200 border border-gray-800 rounded-custom p-4">
                                <div className="flex justify-between items-start mb-3">
                                  <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      <p className="text-sm text-gray-400 truncate">{a.servicos?.nome} • {a.profissionais?.nome}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-center gap-2 shrink-0 ml-2">
                                    {isCancel ? (
                                      <div className="px-3 py-1 rounded-button text-xs bg-red-500/20 text-red-300">CANCELADO</div>
                                    ) : isDone ? (
                                      <div className="px-3 py-1 rounded-button text-xs bg-green-500/20 text-green-400">CONCLUÍDO</div>
                                    ) : isFuturo ? (
                                      <div className="px-3 py-1 rounded-button text-xs bg-yellow-500/20 text-yellow-300">FUTURO</div>
                                    ) : (
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-4">
                                  <div>
                                    ...
                                    <div className="text-sm">{formatDateBRFromISO(getAgDate(a))}</div>
                                  </div>
                                  <div>
                                    <div className="text-xs text-gray-500">HORÁRIO</div>
                                    <div className="text-sm">{getAgInicio(a)}</div>
                                  </div>
                                  <div>
                                    <div className="text-xs text-gray-500">VALOR</div>
                                    <div className="text-sm">R$ {Number(valorReal).toFixed(2)}</div>
                                  </div>
                                </div>
                                ...
                                {!isDone && !isCancel && (
                                  isHoje ? (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                      ...
                                      <button
                                        onClick={() => cancelarAgendamentoProfissional(a.id)}
                        ...
                        return (
                          <div key={a.id} className="bg-dark-200 border border-red-500/30 rounded-custom p-4">
                            ...
                            <div className="grid grid-cols-3 gap-4 text-sm">
                              <div>
                                <div className="text-xs text-gray-500">DATA</div>
                                <div className="text-white">{formatDateBRFromISO(getAgDate(a))}</div>
                              </div>
                              <div>
                                <div className="text-xs text-gray-500">HORÁRIO</div>
                                <div className="text-white">{getAgInicio(a)}</div>
                              </div>
                              <div>
                                <div className="text-xs text-gray-500">VALOR</div>
                                <div className="text-white">R$ {Number(valorReal).toFixed(2)}</div>
                              </div>
                            </div>
                          </div>
                        );
              ...
              <div>
                ...
                {agendamentosDiaSelecionado.length > 0 ? (
                      ...
                      return (
                        <div key={a.id} className={`bg-dark-200 border rounded-custom p-4 ${isCancel ? 'border-red-500/30' : 'border-gray-800'}`}>
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <div className={`px-3 py-1 rounded-button text-xs shrink-0 ml-2 ${
                  ...
                  <div className="text-gray-500 text-center py-12">
                    Nenhum agendamento carregado para essa data.
                  </div>
                )}

                {historicoHasMore && (
                  <button
                    onClick={loadMoreHistorico}
                      ...
                      return (
                        <div key={p.id} className="bg-dark-200 border border-gray-800 rounded-custom p-6">
                          ...
                          {lista.length ? (
                            ...
                            <p className="text-gray-500">Sem serviços para este profissional.</p>
                          )}
                        </div>
                      );
                    })}
                    ...
                    return (
                      <div key={p.id} className="relative bg-dark-200 border border-gray-800 rounded-custom p-5">
                        <div className="flex items-start gap-3 mb-3">
                          <div className="w-12 h-12 bg-gradient-to-br from-primary to-yellow-600 rounded-custom flex items-center justify-center text-normal font-normal text-xl shrink-0">
                            {p.nome?.[0] || 'P'}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-normal flex items-center gap-2">
                              {p.nome}
                              {!ativo && (
                                <span className="absolute top-3 right-3 text-[10px] px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-300 font-normal uppercase">
                                  INATIVO
                                </span>
                              )}
                            </h3>
                        ...
                        <div className="text-xs text-gray-500 mb-3">
                          ...
                          {p.almoco_inicio && p.almoco_fim && (
                            <span className="ml-2 text-yellow-300">• {p.almoco_inicio} - {p.almoco_fim}</span>
                          )}
                        </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-dark-200 border border-gray-800 rounded-custom p-5 md:col-span-2">
                    ...
                    <p className="text-[12px] text-yellow-300 mt-2">
                      Use o formato: <span className="text-gray-300">"RUA, NÚMERO - CIDADE, ESTADO"</span>
                ...
                <div className="bg-dark-200 border border-gray-800 rounded-custom p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <label className="inline-block">
                      ...
                      />
                      <span className={`inline-flex items-center gap-2 rounded-button font-normal border cursor-pointer transition-all uppercase ${
                        galleryUploading
                          ? 'bg-gray-900 border-gray-800 text-gray-600 cursor-not-allowed'
                          : 'bg-primary/20 hover:bg-primary/30 border-primary/50 text-primary'
                      ...
                      {galeriaItems.map((item) => (
                        <div key={item.id || item.path} className="relative bg-dark-100 border border-gray-800 rounded-custom overflow-hidden">
                          ...
                          <button
                            ...
                            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 sm:left-2 sm:top-2 sm:right-auto sm:transform-none px-3 py-1 rounded-full bg-black/60 border border-gray-700 hover:border-red-400 text-[12px] text-red-200 font-normal uppercase"
                          >
                            REMOVER
                          </button>
                        </div>
          ...
          <div className="bg-dark-100 border border-gray-800 rounded-custom max-w-md w-full p-8 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={editingServicoId ? updateServico : createServico} className="space-y-4">
              ...
              <div>
                ...
                <input type="number" step="0.01" value={formServico.preco_promocional} onChange={(e) => setFormServico({ ...formServico, preco_promocional: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" placeholder="Apenas se houver oferta" />
                <p className="text-[12px] text-gray-500 mt-2">O preco de oferta deve ser menor que o preco normal.</p>
              </div>
            ...
            <form onSubmit={editingProfissional ? updateProfissional : createProfissional} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <InputWithChevron>
                    <input type="time" value={formProfissional.horario_inicio} onChange={(e) => setFormProfissional({ ...formProfissional, horario_inicio: e.target.value })} className="no-native-indicator w-full px-4 py-3 pr-12 bg-dark-200 border border-gray-800 rounded-custom text-white" required />
                  </InputWithChevron>
                </div>
                <div>
                  <label className="block text-sm mb-2">Até</label>
                  <InputWithChevron>
                    <input type="time" value={formProfissional.horario_fim} onChange={(e) => setFormProfissional({ ...formProfissional, horario_fim: e.target.value })} className="no-native-indicator w-full px-4 py-3 pr-12 bg-dark-200 border border-gray-800 rounded-custom text-white" required />
                  </InputWithChevron>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <InputWithChevron>
                    <input type="time" value={formProfissional.almoco_inicio} onChange={(e) => setFormProfissional({ ...formProfissional, almoco_inicio: e.target.value })} className="no-native-indicator w-full px-4 py-3 pr-12 bg-dark-200 border border-gray-800 rounded-custom text-white" />
                  </InputWithChevron>
                </div>
                <div>
                  <label className="block text-sm mb-2">Almoço (Fim)</label>
                  <InputWithChevron>
                    <input type="time" value={formProfissional.almoco_fim} onChange={(e) => setFormProfissional({ ...formProfissional, almoco_fim: e.target.value })} className="no-native-indicator w-full px-4 py-3 pr-12 bg-dark-200 border border-gray-800 rounded-custom text-white" />
                  </InputWithChevron>
                </div>
              </div>
                    ...
                    return (
                      <button
                        ...
                        className={`py-2 rounded-button border font-normal text-xs transition-all ${
                          active ? 'bg-primary/20 border-primary/50 text-primary' : 'bg-dark-200 border-gray-800 text-gray-500'
                        }`}
                      >
                        {d.label}
                      </button>
